...
def post():
    ...
    return render_template('posten.html', title='Home Page', form=form, posts=posts.items,next_url=next_url,prev_url=prev_url)
#neu
...
def category(category_id):
    # Kategorie-IDs und Namen in einem Dictionary festlegen
    category_data = {
        1: "Schweiz",
        2: "Ausland",
        3: "Sport",
    ...
    page = request.args.get("page", 1, type=int)
    posts = Post.query.filter_by(category_id=category_id).order_by(Post.timestamp.desc()).paginate(
        page=page, per_page=app.config["POSTS_PER_PAGE"], error_out=False
    )
    next_url = url_for("category", category_id=category_id, page=posts.next_num) if posts.has_next else None
    prev_url = url_for("category", category_id=category_id, page=posts.prev_num) if posts.has_prev else None
    return render_template(
        "index.html", title=f"Category: {category_name}", posts=posts.items, next_url=next_url, prev_url=prev_url
    )
#neu
